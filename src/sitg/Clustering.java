...
public class Clustering {
  ...
  public static void main(String args[]) throws Exception {
	...
	if (args.length != 5)
		System.out.println("Usage: java sitg.TestKmeans k maxNGram minSupport minDF maxDFPercent");
	  
    // inputDir contains the text files
    String inputDir = "inputDir";
    // seqDir contains the sequence files
    String seqDir = "seqDir";    
    
    // creating sequence files from text files
    ...
    */
    ...
    // tokenizing the text files using custom analyzer
    ...
    // reading tokenized documents
    SequenceFile.Reader token_reader = new SequenceFile.Reader(fs,
        new Path(tokenizedPath + "/part-m-00000"), conf);    
    // writing tokenized documents into a file 
    PrintWriter token_out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream("tokens.txt"), "UTF8")));
    // creating a hashtable of tokens
    ...
    // creating term frequency and document frequency vectors
    ...
    int maxNGramSize = Integer.parseInt(args[1]); // [1-5] default:1
    ...
    // creating a hashtable of dictionary
    Hashtable<Integer, String> dictionary = new Hashtable<Integer, String>();
    // writing the dictionary into a file
    PrintWriter dictionary_out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream("dictionary.txt"), "UTF8")));
    ...
    // writing term frequency into a file 
    PrintWriter tf_out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream("tf_vector.txt"), "UTF8")));  
    ...
    // creating a hashtable of frequency
    Hashtable<Integer, Long> frequency = new Hashtable<Integer, Long>();
    // writing frequency into a file
    PrintWriter frequency_out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream("frequency.txt"), "UTF8")));
    ...
    // writing sorted frequency into a file 
    frequency_out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream("frequency_sorted.txt"), "UTF8")));
    ...
    // creating TFIDF 
    int minDf = Integer.parseInt(args[3]); // [0-9] default:0
    int maxDFPercent = Integer.parseInt(args[4]); // [90, 95, 100] default:100
    ...
    // writing tfidf vector into a file 
    PrintWriter tfidf_out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream("tfidf_vector.txt"), "UTF8")));    
    ...
    // creating centroids of k clusters
    int k = Integer.parseInt(args[0]); // [10-40] default:20
    ...
    // creating centroids using canopy clustering
    //double t1 = 0.9;
    //double t2 = 0.85;
    ...
    // reading clusters to find top terms
    ...
    // reading clustering results and writing to a file
    int numTopTerms = 9; // =10
    PrintWriter writer = new PrintWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream("clusters_"+args[0]+"_"+args[1]+"_"+args[2]+"_"+args[3]+"_"+args[4]+".xml"), "UTF8")));
    ...
    BufferedWriter cluster_out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("clusters_"+args[0]+"_"+args[1]+"_"+args[2]+"_"+args[3]+"_"+args[4]+".txt"), "UTF8"));
    ...
    for (Integer id : clusters.keySet()) {
    	if (clusters.get(id).size() > 20) {
    		// running topdown clustering
    		Path postOutput = new Path(outputDir, "postClusters"); // clusterDir/postClusters
    		Path data = new Path(postOutput, "data"); // clusterDir/postClusters/data
    		org.apache.mahout.clustering.topdown.postprocessor.ClusterOutputPostProcessorDriver.run(clusterOutput, data, true); // runSequential = true
    		File top = new File(data.toString());
    		Path downInput = new Path(data, String.valueOf(id)); // clusterDir/postClusters/data/cluster_id
    		Path downOutput = new Path(postOutput, String.valueOf(id)); // clusterDir/postClusters/cluster_id
    		// number of sub clusters
    		k = clusters.get(id).size() / 20 + 1;
    	    // fixing the seed (avoid randomness for test)
    	    RandomUtils.useTestSeed();
    	    RandomSeedGenerator.buildRandom(conf, vectorsFolder, centroids, k, new CosineDistanceMeasure());    		
    		KMeansDriver.run(conf, downInput, new Path(centroids, "part-randomSeed"),
        		//new Path(centroids, "clusters-0-final"), // used for canopy clustering 
        		downOutput, new CosineDistanceMeasure(), convergenceDelta,
        		maxIterations, runClustering, clusterClassificationThreshold, runSequential);
        	
    		List<Cluster> down_klusters_list = Lists.newArrayList();
    		Hashtable<Integer, Cluster> down_klusters = new Hashtable<Integer, Cluster>();
    		
    		for (ClusterWritable clusterWritable :
    			new SequenceFileDirValueIterable<ClusterWritable>(new Path(downOutput, "clusters-*-final/part-*"), PathType.GLOB, PathFilters.logsCRCFilter(), conf)) {
    	        Cluster cluster = clusterWritable.getValue();
    	        down_klusters_list.add(cluster);
    	        down_klusters.put(cluster.getId(), cluster);
    	    }         
    	        
    		clusterDumper = new ClusterDumper(new Path(seqDir), new Path(downOutput, "clusteredPoints"));
    		Map<Integer, List<WeightedVectorWritable>> down_clusters = clusterDumper.getClusterIdToPoints();
    		
	        // write top clusters
    		cluster_out.write("Cluster " + id + " (" + down_clusters.size() + ")\n");
    		// calculate top terms of a cluster
    		...
    		if (kluster != null) {
    			...
    			while (elements.hasNext()) {
    				Element elem = elements.next();
    				topTerms.put(dictionary.get(elem.index()).replace(" ", "_"), Double.valueOf(df.format(elem.get())));
    			}
    		}
    		...
    		for (Integer down_id : down_clusters.keySet()) {	        
    			cluster_out.write("  Cluster " + id + "_" + down_id + "\n");
    			writer.println("    skos:member sitg:collection_" + id + "_" + down_id + ";");    			
    		}    		
	       	writer.println("    skos:note \"" + topList.subList(0, Math.min(topList.size(), numTopTerms)) + "\".");
	       	writer.println();
	       	
    		for (Integer down_id : down_clusters.keySet()) {
    	        cluster_out.write("Cluster " + id + "_" + down_id + " (" + down_clusters.get(down_id).size() + ")\n");                   
    	        // calculate top terms of a cluster
    	        kluster = down_klusters.get(down_id);
    	        topTerms = new Hashtable<String, Double>();
    	        if (kluster != null) {
    	        	Iterator<Element> elements = kluster.getCenter().iterateNonZero();
    	        	while (elements.hasNext()) {
    	        		Element elem = elements.next();
    	        ...
    	        topList = new ArrayList(topTerms.entrySet());
    	        Collections.sort(topList, new TopTermComparator());
    	        cluster_out.write("TopTerms: " + topList.subList(0, Math.min(topList.size(), numTopTerms)) + "\n");
    	       
    	        writer.println("sitg:collection_" + id + "_" + down_id + " rdf:type skos:Collection;");
    	        	
    	        List<WeightedVectorWritable> cluster = down_clusters.get(down_id); 
    	        ...
    	        while (documents.hasNext()) {
    	        	WeightedVectorWritable doc = documents.next();    		
    	           	...
    	           	String name = doc.getVector().toString().substring(0, index);        	
    	        	cluster_out.write("  " + String.format("%-40s", name) + ":");
    	       		counter++;
    	       		writer.println("    skos:member sitg:concept_" + id + "_" + down_id + "_" + counter + ";");
    	       		...
    	       		while (elements.hasNext()) {
    	       			...
    	       			topTermsDoc.put(word.replace(" ", "_"), Double.valueOf(df.format(elem.get())));
    	        	}
    	        	...
    	        	cluster_out.write("\n");
    	        	buffer += "sitg:concept_" + id + "_" + down_id + "_" + counter + " rdf:type skos:Concept;\n";
    	       		buffer += "    skos:definition \"SITG_XML_COMPLET" + name.replaceAll(".txt", ".xml") + "\";\n";
    ...
    // Harmonic mean: 2ab/(a+b) => (2a/b)(a+1/b)=2a/(ab+1)
    double harmonic_mean = 2*avg_inter/(avg_inter*avg_intra+1);
    System.out.println("Harmonic mean: " + df.format(harmonic_mean));
    // Dunn index: minimal inter-cluster distance to maximal intra-cluster distance
